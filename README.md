...
> __Объектный способ__ (объектная техника) доступа к данным реализован посредством использования объектов __встроенного языка__. `ДокументСсылка.Товары`, `ПолучитьОбъект()`
> __Табличный доступ__ (табличная техника) к данным в «1С:Предприятии» реализован с помощью __запросов к базе данных__, которые составляются на языке запросов. `Запрос.Выполнить()`
...
Платформой всегда устанавливаются управляемые __разделяемые__ блокировки при чтении в объектной технике следующих видов объектов:
...
```bsl
// пример чтения набора записей в объектной технике
Набор = РегистрыСведений.ВсеОбновленияНовостей.СоздатьНаборЗаписей();
Набор.Отбор.ВидОбновления.Установить("Получение новостей");
Набор.Прочитать(); // <- событие TLOCK, неявная транзакция
```

Платформа всегда устанавливает _объектную_ блокировку при вызове `ПолучитьОбъект()`
_Пояснение: блокировка объектная оптимистическая - т.е. сравнивает текущий реквизит ВерсияДанных с прочитанным, подробнее см. [Какие бывают блокировки](#какие-бывают-блокировки)_

Платформа НЕ устанавливает _(управляемые)_ блокировки при чтении в объектной технике как минимум следующих объектов:
